...
from logdeep.models.lstm import dislog
...
def train_and_test_time(dataset, CASE, node_list, time_window_size):
    result_file = open(f'result_time_predict_DisLog_{CASE}.log', "w")
    ...
    for node in node_list:
        ...
        options['model_name'] = "dislog"
        options['save_dir'] = "../result/dislog/"

        # Predict
        options['model_path'] = f"../result/dislog/dislog_last_{CASE}_{node}.pth"
        ...
        Model = dislog(input_size=options['input_size'],
        ...
        result.append(predicter.predict_supervised(result_file, time_list, time_index2block, instances))
    result_file.close()
    return result


def train_and_test_time_single_node(dataset, CASE, NODE, time_window_size):
    result_file = open(f'result_time_predict_DisLog_{CASE}_{NODE}.log', "w")
    ...
    trainer.start_train()
    predicter = Predicter(Model, options)
    result = predicter.predict_supervised(result_file, time_list, time_index2block, instances)
    result_file.close()
    return result
