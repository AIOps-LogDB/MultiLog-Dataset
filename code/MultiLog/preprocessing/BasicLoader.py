...
class BasicDataLoader():
    ...
    def parse_by_IBM(self, config_file, persistence_folder, core_jobs=5):
        ...
        log_npz_file = os.path.join(persistence_folder, 'seq_label.npz')
        ...
        if self._check_parsing_persistences(log_template_mapping_file, log_event_seq_file):
            ...
            self._save_as_npz(log_npz_file)
    ...
    def _save_as_npz(self, npz_file):
        self.logger.info('Start saving log event sequences with label to npz.')
        self.logger.info(npz_file)
        x_data = np.empty(len(self.block2eventseq), dtype=list)
        y_data = np.empty(len(self.block2eventseq), dtype=int)
        for block, event_seq in self.block2eventseq.items():
            x_data[int(block)] = event_seq
            y_data[int(block)] = self.label2id[self.block2label[block]]
        np.savez(npz_file, x_data=x_data, y_data=y_data)
        self.logger.info('Log event sequences with label saved.')
    ...
    def _load_log_event_seqs(self, reader):
        for line in reader.readlines():
            ...
            self.block2eventseq[block] = []
            for x in seq:
                self.block2eventseq[block].append(int(x))
