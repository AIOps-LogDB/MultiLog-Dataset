    ...
    def __init__(self, model, options):
        ...
        if self.sample == 'session_window':
            ...
            train_logs, train_labels = session_window(self.data_dir,
                                                      datatype='train', case=self.case, node_index=self.node_index,
                                                      dataset=self.dataset, sequentials=self.sequentials,
                                                      quantitatives=self.quantitatives, semantics=self.semantics,
                                                      multi_class=options['num_classes'] > 2)
            # val_logs, val_labels = session_window(self.data_dir,
            #                                       datatype='val', case=self.case, node_index=self.node_index,
            #                                       dataset=self.dataset, sequentials=self.sequentials,
            #                                       quantitatives=self.quantitatives, semantics=self.semantics)
        else:
